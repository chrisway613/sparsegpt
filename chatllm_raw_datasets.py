...
class DahoasSyntheticinstructgptjpairwiseDataset(PromptRawDataset):
    ...
    def get_train_data(self):
        from chatllm_data_utils import get_raw_dataset_split_index
...
class ShareGPTDataset(PromptRawDataset):
    def split_data(self, raw_data):
        results = []
        for idx, sample in enumerate(raw_data):
            assert len(sample["conversations"]) % 2 == 0
            
            num_of_conv = len(sample["conversations"])
            for idx in range(num_of_conv):
                if idx % 2 == 0:
                    assert sample["conversations"][idx]["from"] == "human"
                if idx % 2 == 1:
                    assert sample["conversations"][idx]["from"] == "gpt"

            for idx in range(0, num_of_conv, 2):
                conv_id = f"{sample['id']}_{idx/2}"
                if idx+2 > num_of_conv:
                    break
                stop_idx = idx+2
                results.append({"id":conv_id, "conversations": sample["conversations"][idx:stop_idx]})      
        return results
    
    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, None)
        self.dataset_name = dataset_name
        self.dataset_name_clean = "ShareGPTDataset"
        
        train_data_path, eval_data_path = dataset_name.split('|')
        
        self.train_raw_data = self.split_data(json.load(open(train_data_path, "r")))
        self.eval_raw_data = self.split_data(json.load(open(eval_data_path, "r")))
        
    def get_train_data(self):
        return self.train_raw_data

    def get_eval_data(self):
        return self.eval_raw_data

    # The prompt should be in the format of: " Human: " + actual_prompt_sentence + " Assistant:"
    def get_prompt(self, sample):
        return " Human: " + sample['conversations'][0]["value"] + " Assistant:"
    ...
    def get_prompt_and_chosen(self, sample):
        return " Human: " + sample['conversations'][0]["value"] + " Assistant: " + sample['conversations'][1]["value"]
...
class InstructionDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, None)
        self.dataset_name = dataset_name
        self.dataset_name_clean = "InstructionDataset"
        
        train_data_path, eval_data_path = dataset_name.split('|')
        
        self.train_raw_data = json.load(open(train_data_path, "r"))
        self.eval_raw_data = json.load(open(eval_data_path, "r"))
